...
export default async (client) => {
	const data = await client
			.fetch("https://livida.net/api/radio/")
			.then((res) => res.data),
		title =
			data.nowplaying.song.name.length > 30
				? data.nowplaying.song.name.substring(0, 27) + "..."
				: data.nowplaying.song.name,
		...
		colours = await Vibrant.from(data.nowplaying.album.art)
			.maxColorCount(2)
			.getPalette();

	registerFont(`${process.cwd()}/assets/OpenSans-Bold.ttf`, {
		family: "OpenSans Bold",
	});
	registerFont(`${process.cwd()}/assets/OpenSans-Regular.ttf`, {
		family: "OpenSans",
	});
	...
	client.nowPlaying = new Canvas(1630, 670)
		...
		.lineTo(0, 0)
		.stroke()
		.printLinearColorGradient(815, 0, 815, 670, [
			{ position: 0, color: colours.LightVibrant.getHex() },
			{ position: 100, color: colours.DarkVibrant.getHex() },
		])
		.fill()
		.restore()
