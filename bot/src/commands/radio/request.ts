...
class RequestCommand extends Command {
	...
	showResults(msg, results) {
		...
		sure.on("collect", (_, member) => {
			if (member.id !== message.author.id)
				return msg.reactions.cache
					.find((r) => r._emoji.name == "✅")
					.users.remove(message.author.id);
			responded = true;
			msg.reactions.removeAll();
			msg.edit({
				embed: {
					description: `${message.author}, your request has been submitted successfully!`,
					color: "GREEN",
				},
			});
			...
			client.requestSong(
				results.data[result - 1].artist.name +
					" - " +
					results.data[result - 1].title,
				message.author.tag
			);
		});
		...
		no.on("collect", (_, member) => {
			...
			msg.edit({
				embed: {
					description: `${message.author}, cancelling request!`,
					color: "RED",
				},
			});
			msg.delete({ timeout: 3000 });
	...
	nextPage(direction: number) {
		...
		embed = {
			description: `Showing result ${result} of ${
				results.total
			} for query \`${
				query.substr(0, 25) + (query.length > 25 ? "..." : "")
			}\``,
			color: "GREEN",
			thumbnail: {
			...
			fields: [
				{
					name: "Track",
					value: `[${results.data[result - 1].title}](${
						results.data[result - 1].link
					}) by [${results.data[result - 1].artist.name}](${
						results.data[result - 1].artist.link
					})`,
					inline: false,
				},
				{
			...
			} - View more info about this song`,
		};
	...
	noResults(msg: Message) {
		const embed = {
			description:
				"No tracks were found relating to your search query, submit request anyways?",
			color: "RED",
		};
		...
		msg.edit({ embed }).then((_) => {
			...
			const yes = (reaction) => reaction.emoji.name == "✅",
				nopleas = (reaction) => reaction.emoji.name == "❌",
				sure = msg.createReactionCollector(yes, { time: 1000000 }),
				no = msg.createReactionCollector(nopleas, {
					time: 1000000,
				});
			...
			setTimeout(() => {
				...
				msg.edit({
					embed: {
						description: `${message.author}, your request has timed out!`,
					},
				});
				msg.delete({ timeout: 3000 });
				message.delete({ timeout: 3000 });
	...
	async run(message: Message, args: string[]) {
		...
		message.channel.send({ embed }).then((msg) => {
			...
			client
				.fetch(`https://api.deezer.com/search?q=${searchParams}`)
				.then((res) => res.data)
				.then((json) => {
