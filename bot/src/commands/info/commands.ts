module.exports = {
	...
	run: (client, message, args, prefix) => {
		const modules = [...new Set(client.commandCategories)],
			mAliases = [].concat.apply(
				[],
				client.Constants.commandCategories.map((x) => x.aliases)
			);
		if (client.devs.includes(message.author.id)) modules.push("Developer");
		if (!args[0]) {
			const embed = new client.Embed()
				.setAuthor(
					`${client.user.username} • Command Categories`,
				...
				.setFooter(
					`${client.commands.size} total commands - <required> [optional]`
				);
			modules.forEach((module) =>
				embed.addField(
					`${client.moduleEmoji[module as any]} **${module}**`,
					`${
						client.commands
			...
			let embed = new client.Embed().setDescription(
			...
			message.channel.send(embed).then((msg) => {
				msg.react(client.Constants.Emojis.Numbers.One).then(() => {
					...
					const cmd = (reaction, user) =>
							reaction.emoji.name ==
								client.Constants.Emojis.Numbers.One &&
							user.id === message.author.id,
						cat = (reaction, user) =>
							reaction.emoji.name ==
								client.Constants.Emojis.Numbers.Two &&
							user.id === message.author.id,
					...
					category.on("collect", () =>
						categoryEmbed(
							...
							client.commands.filter(
								(cmd) =>
									cmd.config.category.toLowerCase() ==
									client.Constants.commandCategories
										.filter((m) =>
											m.aliases.includes(
												args[0].toLowerCase()
											)
							...
							client.Constants.commandCategories.filter((m) =>
			...
			var cat = client.Constants.commandCategories.filter((m) =>
				m.aliases.includes(args[0].toLowerCase())
			)[0].name;
			let commands = client.commands.filter(
...
function categoryEmbed(client, message, _, commands, category?, msg?) {
	const embed = new client.Embed()
		...
		.setDescription(
			commands.map((cmd) => `\`${cmd.config.name}\``).join(", ")
		)
